...
simulateHumiditySensor = function () {
    ...
    $.get("/humidityMonitor/", {read: "sensorRead"}, function (data) {
        ...
        for (var i = 0; i < workingSensors.length; i++) {
            ...
            $.post("/humidityMonitor/", {writeType: "sensorWrite", id:id});//用sensorName是为了与编辑操作的公用服务器端操作的request.POST['sensorName']保持一致
        }
    },"JSON")
};
...
simulateatomPressSensor = function () {
    ...
    $.get("/atomPressMonitor/", {read: "sensorRead"}, function (data) {
        ...
        for (var i = 0; i < workingSensors.length; i++) {
            ...
            //上传数据到服务器,不能直接传一个sensor对象，只好拆开,但只需部分数据
            var id = newSensor.id;
            //var atomPress = newSensor.atomPress;
...
//                    alert("设备：【" + data.allSensorsJSON[i].name + "】 监测到的亮度过高！" + "\n为：" + data.allSensorsJSON[i].luminance + " lm" + "\n超过399 lm限制！");
                ...
                htmlstr = htmlstr +
                    ...
                    "<td style='text-align:center'>" + data.allSensorsJSON[i].luminance + " lux" + "</td>" +
...
//                    alert("设备：【" + data.allSensorsJSON[i].name + "】 监测到的湿度过高！" + "\n为：" + data.allSensorsJSON[i].humidity + "%" + "\n超过99%限制！");
                ...
                htmlstr = htmlstr +
                    ...
                    "<td style='text-align:center'>" + data.allSensorsJSON[i].name + "</td>" +
                    "<td style='text-align:center'>" + deviceStatusString + "</td>" +
                    "<td style='text-align:center'>" + data.allSensorsJSON[i].humidity + " RH" + "</td>" +
...
//                    alert("设备：【" + data.allSensorsJSON[i].name + "】 监测到的湿度过高！" + "\n为：" + data.allSensorsJSON[i].humidity + "%" + "\n超过99%限制！");
                ...
                htmlstr = htmlstr +
                    ...
                    "<td style='text-align:center'>" + data.allSensorsJSON[i].name + "</td>" +
                    "<td style='text-align:center'>" + deviceStatusString + "</td>" +
                    "<td style='text-align:center'>" + data.allSensorsJSON[i].atomPress + " kpa" + "</td>" +
...
$(document).ready(function () {
    var showType = "null";
    var temSim, floSim, humSim, atoSim;
    var temRel, floRel, humRel, atoRel;
    ...
    $("#floodlightShowBtn").click(function () {
        ...
        }else if("atomPress" == showType){
            window.clearInterval(atoSim);
            window.clearInterval(atoRel);
        }
    ...
    $("#atomPressShowBtn").click(function () {
        $("#realTimeData").text("实时气压");
        // $("#humidityShowBtn").attr("class", "btn btn-primary");
        // $("#floodlightShowBtn").attr("class", "btn btn-danger");
        // $("#temperatureShowBtn").attr("class", "btn btn-danger");
       ...
       if ("floodlight" == showType){
        ...
        atoSim = window.setInterval(simulateatomPressSensor, 2500);
        atoRel = window.setInterval(reloadatomPressTable, 2000);
    });
    ...
    $("#addSensor").click( function () {
        if ("null" == showType){
            ...
            // 设置title
            if ("floodlight" == showType)
                $('.modal-title').text('新建光照传感器');
            else if ("temperature" == showType)
                $('.modal-title').text('新建温度传感器');
            else if ("humidity" == showType)
                $('.modal-title').text('新建湿度传感器');
            else if ("atomPress" == showType)
                $('.modal-title').text('新建气压传感器');
        }
